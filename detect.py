...
def run_image_detection(model_path, source, conf, save_dir, save_txt):
    ...
    for result in tqdm(results, desc="Processing images", unit="image"):
        ...
        save_detection_results(result, save_dir, img_name, save_txt)
...
def run_detection(model_path, source, conf, save_dir, save_txt):
    ...
    if os.path.isfile(source):
        if is_video_file(source):
            ...
            save_detection_results(results[0], save_dir, os.path.basename(source), save_txt)
        ...
        with tqdm(total=len(files), desc="Processing files", unit="file") as pbar:
            for filename in files:
                ...
                if is_video_file(filepath):
                    ...
                    save_detection_results(results[0], save_dir, filename, save_txt)
                pbar.update(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default=r'weights\ultralytics\yolo26n.pt', help='Path to model weights (use hand detection weights for hand tracking)')
    parser.add_argument('--source', type=str, default=r'C:\Users\Administrator\Desktop\新建文件夹 (2)', help='Source directory, image path, or video file path')
